...
class App extends BaseApp {
    ...
    public function init() {
        ...
        add_filter( 'static_archive_post_types', [ self::class, 'add_static_archive_post_type' ] );
        add_filter( 'static_archive_post_html', [ $this, 'static_archive_recipe_html' ], 10, 3 );
        add_filter( 'static_archive_post_markdown', [ $this, 'static_archive_recipe_markdown' ], 10, 3 );
    ...
    private function render_static_archive_recipe_html( \WP_Post $post ): string {
        ...
        $prep         = (int) get_post_meta( $id, self::META_PREP, true );
        $cook         = (int) get_post_meta( $id, self::META_COOK, true );
        ...
        $instructions = $this->clean_static_archive_instructions( (array) get_post_meta( $id, self::META_INSTRUCTIONS, true ) );
        ...
        $term_groups = [
            __( 'Categories', 'cookbook' ) => wp_get_object_terms( $id, self::TAX_CATEGORY ),
            __( 'Cuisines', 'cookbook' )   => wp_get_object_terms( $id, self::TAX_CUISINE ),
            __( 'Tags', 'cookbook' )       => wp_get_object_terms( $id, self::TAX_TAG ),
        ];
        $term_lines = [];
        ...
        if ( $ingredients ) {
            ...
            foreach ( $ingredients as $ingredient ) {
                ...
                $line = self::static_archive_ingredient_text( $ingredient );
                if ( $line !== '' ) {
    ...
    private function render_static_archive_recipe_markdown( \WP_Post $post ): string {
        ...
        if ( $thumbnail_url ) {
            $sections[] = '![' . self::static_archive_markdown_text( get_the_title( $post ) ) . '](' . esc_url_raw( $thumbnail_url ) . ')';
        }
        ...
        if ( $meta ) {
            $sections[] = implode( "\n", array_map( fn( $line ) => '- ' . self::static_archive_markdown_text( $line ), $meta ) );
        }

        $description = self::static_archive_markdown_text( $post->post_content );
        if ( $description !== '' ) {
        ...
        foreach ( $ingredients as $ingredient ) {
            ...
            $line = self::static_archive_ingredient_text( $ingredient );
            if ( $line !== '' ) {
                $ingredient_lines[] = '- ' . self::static_archive_markdown_text( $line );
            }
        }
        ...
        foreach ( $instructions as $index => $step ) {
            $text = self::static_archive_markdown_text( $step );
            if ( $text !== '' ) {
        ...
        $notes_text = self::static_archive_markdown_text( $notes );
        if ( $notes_text !== '' ) {
    ...
    private static function static_archive_ingredient_text( array $ingredient ): string {
    ...
    private static function static_archive_markdown_text( string $value ): string {
