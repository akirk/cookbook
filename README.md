...
- **Quick ingredient replacement.** Replace an imported ingredient directly from the recipe view; ingredient meta is revisioned so the change can be tracked through WordPress revisions.
- **Shopping list.** Add a recipe's scaled ingredients to your personal shopping list, edit the list at home, and use a focused shop mode with large tap targets, hide-checked, undo, and quick add.
